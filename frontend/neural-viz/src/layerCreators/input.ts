...
const makeParentLayerNode = (modelId: string, position: XYPosition, width: number, height: number, numChannels: number, handleDirection: Direction): Node => {
    return {
        ...
        height: height,
        data: {
            label: `Input (${numChannels} channels)`,
            layerType: 'Input' as const,
...
const makeInputChannelNode = (
    ...
    return {
        ...
        position: position,
        data: {
            // Coordinate for input is just "x.out_X" as set up in pt-to-api
            coordinate: `x.out_${channelIndex}`,
