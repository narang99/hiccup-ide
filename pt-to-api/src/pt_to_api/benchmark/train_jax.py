...
# from .init_strats import SingleRun, InitStrategy, StandardInitStrategy, NoInitStrategy
# from .train_x import (
...
# @nnx.vmap(in_axes=(0, 0, 0, None, None, None, None), out_axes=0)
...
#     best_losses = [float("inf") for _ in range(n_models)]
...
#         for key, batch in get_batches(key, X_jax, batch_size):
...
#             if nan_detected(
#                 n_models, all_recon_losses, all_weight_losses, all_codes_losses
#             ):
#                 raise Exception(
#                     f"NaN detected in model {j}: recon={all_recon_losses[j]:.4f}"
#                 )
...
#             ]
#             for model_idx, loss in enumerate(avg_losses):
#                 if best_losses[model_idx] > loss:
#                     best_losses[model_idx] = 0
...
# def get_batches(key, X, batch_size):
...
# def nan_detected(n_models, all_recon_losses, all_weight_losses, all_codes_losses):
#     for j in range(n_models):
#         if (
