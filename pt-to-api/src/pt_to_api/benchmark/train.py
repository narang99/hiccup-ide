...
def get_hyperparameters_and_init(
    model, X, n_components, input_dim, sigma_eps, svd_init=False, sigma_s_rel_to_0="equal", device="cpu"
):
    sigma_x = X.std()
...
def train(
    ...
    batch_size=2048,
    verbose=True,
    init_strategy:SvdInitStrategy | WarmupInitStrategy | StandardInitStrategy=StandardInitStrategy(),
    svd_init=False,
    ...
    baseline_run = train_baseline(X, n_components, lr, baseline_epochs, batch_size, False)
    ...
    if isinstance(init_strategy, WarmupInitStrategy) or isinstance(init_strategy, StandardInitStrategy):
        svd_init = False
        scaled_hyperparameters = get_hyperparameters_and_init(
            model, X, n_components, input_dim, sigma_eps, svd_init, sigma_s_rel_to_0, device
        )
    elif isinstance(init_strategy, SvdInitStrategy):
        svd_init = True
        scaled_hyperparameters = get_hyperparameters_and_init(
            model, X, n_components, input_dim, sigma_eps, svd_init, sigma_s_rel_to_0, device
        )
    else:
        raise Exception(f"invalid init_stragegy={init_strategy}")
