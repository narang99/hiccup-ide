...
def train(
    ...
    if not isinstance(
        init_strategy, (StandardInitStrategy, NoInitStrategy, ZeroInitStrategy)
    ):
    ...
    for epoch in range(epochs):
        ...
        alpha = alpha_schedule.get_multiplier(epoch, epochs) * p["alpha"]
        uncond_params = get_uncond(
            recon_err_multiplier=recon_err_multiplier,
            epoch_mod=epoch_mod,
            alpha=alpha,
        )
