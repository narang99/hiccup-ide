import pytest
from pt_to_api.activation_processor import (
...
def test_activation_capture_includes_input_channels(model_snapshot):
    ...
    first_conv_coords = [k for k in input_channel_coords if k.startswith("layers.0") and not k.endswith(".input")]
    ...
    second_conv_coords = [k for k in input_channel_coords if k.startswith("layers.2") and not k.endswith(".input")]
