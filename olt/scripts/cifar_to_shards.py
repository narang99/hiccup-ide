"""Convert CIFAR-10 into the webdataset .tar shard layout the pipeline expects.
...
with keys `__key__` and `jpg`. For ImageNet that class id is the imagenet
label; for CIFAR it's simply the class 0..9 — no stage signatures change, only
the data on disk.
...
Output layout (mirrors the ImageNet image-shard layout):
    ...
    <out_dir>/<class_id>/000000.tar
        ...
        jpg     = <32x32 RGB JPEG bytes>

Usage:
    uv run scripts/cifar_to_shards.py --out-dir data/cifar/image-shards --split test
    uv run scripts/cifar_to_shards.py --out-dir data/cifar/image-shards --split train

The `test` split is the natural analogue of the ImageNet inputs the pipeline
was run over (held-out inputs to probe); use `train` if you also want to probe
training images. `--flat-image-dir` optionally also dumps every image as
...
def convert(
    ...
    for class_id, idxs in tqdm(sorted(by_class.items()), desc=f"cifar-{split}"):
